...
public class MoreQikuFragment extends Fragment implements INotifyAddRes, IItemMoreItem {
    private static final String TAG = MoreQikuFragment.class.getSimpleName();
    ...
    private void initView() {
        View header = LayoutInflater.from(getContext()).inflate(R.layout.header_more_wuqiku, null);
        GridView gridView = (GridView) header.findViewById(R.id.header_more_gv);
        data = WushubaikeModel.getWushubaike().兵器库;
        ...
        JsonParseAndUpdate.moreItemJPAU(adapter, HttpUrl.MORE_QIKU+page,JsonParseAndUpdate.UPDATE);
    }

    @Override
    public void addResForI() {
        page++ ;
        JsonParseAndUpdate.moreItemJPAU(adapter,HttpUrl.MORE_QIKU+page,JsonParseAndUpdate.ADD);
    }
